...
$col_check = $conn->query("SHOW COLUMNS FROM client_records LIKE 'deo_done_at'");
...
<script>
    ...
    function checkSession() {
        ...
        $.post('api.php', {action:'check_session'}, function(res){
            if(res.status==='logged_in'){
                ...
                if(res.role === 'supervisor') { window.location.href = 'p1_admin_dashboard.php'; return; }
                currentUser = res;
    ...
    function renderSidebar() {
        ...
        pageData.forEach(r=>{
            ...
            if (r.row_status === 'done') { statusClass = 'status-done'; itemClass = 'done';
                let tsDone = r.deo_done_at || r.updated_at;
                if(tsDone) qcByInfo = '<br><small style="color:#27ae60;">1st QC Done: '+fmt12hr(tsDone)+'</small>';
            }
                ...
                let tsqc = [];
                let ts1q = r.deo_done_at; if(ts1q) tsqc.push('<small style="color:#27ae60;">1st QC: '+fmt12hr(ts1q)+'</small>');
                let ts2q = r.qc_done_at || r.updated_at; if(ts2q) tsqc.push('<small style="color:#0d6efd;">2nd QC: '+fmt12hr(ts2q)+'</small>');
                if(tsqc.length) qcByInfo = '<br>' + tsqc.join(' ');
            }
            else if (r.row_status === 'Completed') { 
                statusClass = 'status-completed'; itemClass = 'completed';
                let tsc = [];
                if(r.deo_done_at) tsc.push('<small style="color:#27ae60;">1st: '+fmt12hr(r.deo_done_at)+'</small>');
                if(r.qc_done_at) tsc.push('<small style="color:#0d6efd;">2nd: '+fmt12hr(r.qc_done_at)+'</small>');
                let tsDone2 = r.completed_at || r.updated_at; if(tsDone2) tsc.push('<small style="color:#dc3545;">Done: '+fmt12hr(tsDone2)+'</small>');
                if(tsc.length) qcByInfo = '<br>' + tsc.join(' ');
            }
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if(e.ctrlKey && e.key === 'i') {
            e.preventDefault();
            if(activeRecord) openImage(activeRecord.record_no);
        }
