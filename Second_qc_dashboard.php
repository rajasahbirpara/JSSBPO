...
// Create QC settings table if not exists (MUST be before the check below)
...
// One-time schema check for QC columns
$qc_col_check = $conn->query("SHOW COLUMNS FROM client_records LIKE 'qc_locked_by'");
if ($qc_col_check && $qc_col_check->num_rows == 0) {
...
<script>
        ...
        $.post('api.php', {action:'check_session'}, function(res){
            if(res.status==='logged_in'){
                ...
                if(res.role !== 'qc') { 
                    ...
                    else if(res.role === 'supervisor') { window.location.href = 'p1_admin_dashboard.php'; }
    ...
    function updateActiveRecordBadge() {
        ...
        const s = activeRecord.row_status;
        if (s === 'done' || s === 'deo_done') {
            badgeClass = 'bg-info'; badgeText = 'DEO DONE';
        } else if (s === 'pending_qc') {
            badgeClass = 'bg-primary'; badgeText = 'PENDING QC';
        } else if (s === 'qc_done' || s === 'qc_approved') {
            badgeClass = 'bg-success'; badgeText = 'QC DONE';
        } else if (s === 'Completed') {
    ...
    function refreshSidebarStatuses() {
        $('.record-item').each(function() {
            ...
            if (rec) {
                ...
                const s = rec.row_status;
                if (s === 'done' || s === 'deo_done' || s === 'pending_qc') dot.addClass('status-done');
                else if (s === 'qc_done' || s === 'qc_approved' || s === 'Completed') dot.addClass('status-completed');
    ...
    document.addEventListener('keydown', function(e) {
        ...
        if(e.ctrlKey && e.key === 'i') {
            e.preventDefault();
            if(activeRecord && activeRecord.record_no) openImage(activeRecord.record_no);
        }
