...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_progress'])) {
    ...
    if (is_numeric($record_from) && is_numeric($record_to)) {
        ...
        if (!$is_assigned) {
            ...
            $dup_stmt = $conn->prepare("SELECT id FROM work_logs WHERE deo_id = ? AND record_from = ? AND record_to = ?");
            $dup_stmt->bind_param("iss", $user['id'], $record_from, $record_to);
            $dup_stmt->execute();
            ...
            if ($dup_result->num_rows > 0) {
                ...
                if ($count > 0) {
                    ...
                    foreach ($chunks as $chunk) {
                        ...
                        $sql = "UPDATE client_records SET row_status = 'done', 
                                assigned_to = '{$user['username']}', 
                                assigned_to_id = {$user['id']},
                                updated_at = NOW() 
                                WHERE record_no IN ($in_clause) 
                    ...
                    $log_stmt->bind_param("issi", $user['id'], $record_from, $record_to, $count);
                    $log_stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['mark_corrected'])) {
    ...
    if($get_rec){
        ...
        $conn->query("UPDATE client_records SET row_status='corrected', updated_at=NOW() WHERE record_no='$rec_no'");
    }
            ...
            recordInput.addEventListener('blur', function() {
                ...
                if(val) {
                    ...
                    fetch('api.php', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                        body: 'action=get_image&record_no=' + encodeURIComponent(val)
                    })
                        .then(response => response.json())
                        .then(data => {
