...
function is_user_locked($conn, $user_id) {
    ...
    $res = $stmt->get_result();
    if ($row = $res->fetch_assoc()) {
        if (isset($row['status']) && $row['status'] === 'locked') return true;
...
function handle_failed_login($conn, $user_id, $username) {
    ...
    if ($row = $res->fetch_assoc()) {
        if ($row['failed_attempts'] >= $max_attempts) {
            ...
            $stmt = $conn->prepare("UPDATE users SET status = 'locked', locked_until = ? WHERE id = ?");
            if ($stmt) {
...
try {
    ...
    if ($action == 'autotyper_login') {
        ...
        $sql = "SELECT id, username, password, full_name, `role` FROM users WHERE (username = '$escaped_user' OR email = '$escaped_user') AND is_active = 1 LIMIT 1";
    ...
    if ($action == 'login_init') {
        ...
        if ($row = $res->fetch_assoc()) {
            ...
            if ($has_security_cols) {
                ...
                if ($status === 'locked' || ($locked_until && strtotime($locked_until) > time())) {
                    $remaining = $locked_until ? ceil((strtotime($locked_until) - time()) / 60) : 0;
    ...
    if ($action == 'load_data') {
        ...
        // Admin can see all records, DEO only sees their own
    ...
    if ($action == 'delete_record') {
        ...
        if ($id <= 0) send_json(['status'=>'error', 'message'=>'Invalid record ID']);
        
        $stmt = $conn->prepare("DELETE FROM client_records WHERE id = ?");
        $stmt->bind_param("i", $id);
        if ($stmt->execute() && $stmt->affected_rows > 0) {
            log_activity($conn, $_SESSION['user_id'], $_SESSION['username'], 'delete_record', 'data', $id);
            send_json(['status'=>'success', 'message'=>'Record deleted']);
        } else {
            send_json(['status'=>'error', 'message'=>'Record not found or delete failed']);
        }
    }
    ...
    if ($action == 'batch_mark_completed_by_record_no') {
        // SESSION CHECK REMOVED - Safe for AutoTyper multi-user
    ...
    if ($action == 'upload_images_files') {
        ...
        foreach ($_FILES['images']['tmp_name'] as $i => $tmp) {
            if (is_uploaded_file($tmp) && move_uploaded_file($tmp, "./uploads/" . $_FILES['images']['name'][$i])) $u++;
        }
    ...
    if ($action == 'upload_single_image_and_map') {
        ...
        $name = preg_replace("/[^a-zA-Z0-9\-\_]/", "", pathinfo($_FILES['image']['name'], PATHINFO_FILENAME)) . '.' . pathinfo($_FILES['image']['name'], PATHINFO_EXTENSION);
    ...
    if ($action == 'get_users') {
        ...
        while ($row = $r->fetch_assoc()) {
            ...
            $row['is_locked'] = ($row['status'] === 'locked' || ($row['locked_until'] && strtotime($row['locked_until']) > time())) ? 1 : 0;
            $d[] = $row;
        }
        send_json($d);
    }
    ...
    if ($action == 'get_all_users_performance') {
        ...
        $sql = "SELECT 
                ...
                FROM users u
                LEFT JOIN client_records cr ON u.username = cr.username
                WHERE u.`role` = 'deo'
                GROUP BY u.id
    ...
    if ($action == 'get_monthly_report') {
        ...
        // ✅ Corrected Monthly Query: Target is Total Assigned, Count is Strict Completed
        $sql = "SELECT 
                    u.username, u.full_name,
                    (SELECT COUNT(*) FROM client_records WHERE assigned_to = u.username) as daily_target, 
                    SUM(CASE WHEN cr.row_status = 'Completed' AND DATE_FORMAT(cr.updated_at, '%Y-%m') = ? THEN 1 ELSE 0 END) as completed,
                    SUM(CASE WHEN cr.row_status = 'Completed' AND DATE_FORMAT(cr.updated_at, '%Y-%m') = ? THEN cr.time_spent ELSE 0 END) as total_time,
                    COUNT(DISTINCT CASE WHEN cr.row_status = 'Completed' AND DATE_FORMAT(cr.updated_at, '%Y-%m') = ? THEN DATE(cr.updated_at) END) as working_days
                FROM users u
                LEFT JOIN client_records cr ON u.username = cr.username
                WHERE u.`role` = 'deo'
                GROUP BY u.id
        ...
        // Weekly breakdown - Strict Completed Check
        $sql = "SELECT 
    ...
    if ($action == 'get_monthly_report') {
        ...
        // ✅ CHANGE: Target ab 'Total Assigned Records' hoga
        $sql = "SELECT 
                    u.username, u.full_name,
                    (SELECT COUNT(*) FROM client_records WHERE assigned_to = u.username) as daily_target, -- Total Assigned Data as Target
        ...
        while ($row = $result->fetch_assoc()) {
            $row['avg_time'] = $row['completed'] > 0 ? round($row['total_time'] / $row['completed']) : 0;
            $row['daily_avg'] = $row['working_days'] > 0 ? round($row['completed'] / $row['working_days']) : 0;
            // Monthly progress based on Total Assigned
            $row['target_progress'] = $row['daily_target'] > 0 ? round(($row['completed'] / $row['daily_target']) * 100) : 0;
        ...
        // Weekly breakdown (No changes needed here, logic remains same)
        $sql = "SELECT 
                    WEEK(updated_at) as week_num,
                    MIN(DATE(updated_at)) as week_start,
    ...
    if ($action == 'export_report_csv') {
        ...
        if ($report_type === 'daily') {
            ...
            $sql = "SELECT u.username, u.full_name, u.daily_target,
                    ...
                    SUM(CASE WHEN DATE(cr.updated_at) = ? THEN cr.time_spent ELSE 0 END) as total_time
                    FROM users u LEFT JOIN client_records cr ON u.username = cr.username
    ...
    if ($action == 'send_admin_daily_summary') {
        ...
        $sql = "SELECT u.username, COUNT(cr.id) as completed 
                FROM users u 
                JOIN client_records cr ON u.username = cr.username 
    ...
    if ($action == 'send_low_productivity_alert') {
        ...
        $stmt = $conn->prepare("SELECT u.daily_target, COUNT(cr.id) as completed 
                                FROM users u 
                                LEFT JOIN client_records cr ON u.username = cr.username AND DATE(cr.updated_at) = CURDATE() AND cr.row_status IN ('done', 'Completed')
                                WHERE u.username = ?
                                GROUP BY u.id");
        $stmt->bind_param("s", $username);
        $stmt->execute();
    ...
    if ($action == 'clean_old_logs') {
        ...
        $stmt = $conn->prepare("DELETE FROM login_attempts WHERE created_at < DATE_SUB(NOW(), INTERVAL ? DAY)");
        $stmt->bind_param("i", $days);
        $stmt->execute();
        $deleted_login = $stmt->affected_rows;
        
        log_activity($conn, $_SESSION['user_id'], $_SESSION['username'], 'clean_logs', 'settings', null, null, null, null, "Deleted $deleted_activity activity + $deleted_login login logs older than $days days");
        
        send_json(['status' => 'success', 'message' => "Deleted $deleted_activity activity logs and $deleted_login login attempts older than $days days"]);
    }
    ...
    if ($action == 'toggle_maintenance_mode') {
        ...
        log_activity($conn, $_SESSION['user_id'], $_SESSION['username'], 'toggle_maintenance', 'settings', null, null, null, $enabled);
        
        send_json(['status' => 'success', 'message' => $enabled === '1' ? 'Maintenance mode enabled' : 'Maintenance mode disabled']);
    }
...
class QueryCache {
        ...
        $tables_sql = [
            ...
            'users' => "
                CREATE TABLE IF NOT EXISTS `users` (
                    ...
                    `role` enum('admin','deo','dqc','qc') DEFAULT 'deo',
            ...
            'announcements' => "
                CREATE TABLE IF NOT EXISTS `announcements` (
                    ...
                    `target_role` enum('all','deo','dqc','admin') DEFAULT 'all',
                    `is_active` tinyint(4) DEFAULT 1,
    ...
    // ============================================================
    if ($action == 'tv_dashboard_data') {
        $token = $_POST['token'] ?? $_GET['token'] ?? '';
        $tv_token_res = $conn->query("SELECT setting_value FROM security_settings WHERE setting_key='tv_dashboard_token'");
        $valid_token = ($tv_token_res && $row2 = $tv_token_res->fetch_assoc()) ? $row2['setting_value'] : 'JSSTV2025';
        ...
        ");
        $deos = [];
        if ($deo_res) while ($row2 = $deo_res->fetch_assoc()) {
            $target = max(1,(int)$row2['daily_target']);
            $row2['target_pct']    = min(100, round(($row2['today_done']/$target)*100));
            $row2['display_name']  = !empty($row2['full_name']) ? $row2['full_name'] : $row2['username'];
            $deos[] = $row2;
        }
        ...
        for ($h=7; $h>=0; $h--) {
            ...
            $label = date('h A', strtotime("-{$h} hours"));
            ...
            $hourly[] = ['label'=>$label,'count'=>$cnt];
        }

        $active_time = date('Y-m-d H:i:s', strtotime('-10 minutes'));
        ...
        $active_users = [];
        if ($active_res) while ($row2=$active_res->fetch_assoc())
            $active_users[] = !empty($row2['full_name']) ? $row2['full_name'] : $row2['username'];

        send_json(['status'=>'success','server_time'=>date('d M Y, h:i:s A'),'today'=>date('d M Y'),
            'total'=>$total,'pending'=>$pending,'qc1_pending'=>$qc1done,'qc2_done'=>$qc2done,
            'completed'=>$completed,'today_done'=>$today_done,'progress'=>$progress,
