...
// One-time schema migration per session
if (empty($_SESSION['_p2_schema_checked'])) {
    ...
    $_SESSION['_p2_schema_checked'] = true;
}
...
$today_submissions = $conn->query("SELECT COUNT(*) as count FROM client_records WHERE row_status = 'Completed' AND DATE(updated_at) = CURDATE()")->fetch_assoc()['count'];
...
if ($active_tab == 'submissions') {
    ...
    $filter_conditions = ["cr.row_status = 'Completed'"];
    if (!empty($filter_date)) $filter_conditions[] = "DATE(cr.updated_at) = '{$filter_date}'";
    if (!empty($filter_deo_id_sub)) {
        $deo_user_row = $conn->query("SELECT username FROM users WHERE id = " . intval($filter_deo_id_sub));
        if ($deo_user_row && $deo_user_row->num_rows > 0) {
            $deo_uname = $conn->real_escape_string($deo_user_row->fetch_assoc()['username']);
            $filter_conditions[] = "(cr.assigned_to = '$deo_uname' OR cr.assigned_to_id = " . intval($filter_deo_id_sub) . ")";
        }
    }
    ...
    $submissions_query = "
        SELECT 
            COALESCE(u.full_name, cr.assigned_to) as deo_name,
            MIN(CAST(cr.record_no AS UNSIGNED)) as record_from,
            MAX(CAST(cr.record_no AS UNSIGNED)) as record_to,
            COUNT(*) as record_count,
            MAX(cr.updated_at) as latest_time
        FROM client_records cr
        LEFT JOIN users u ON (cr.assigned_to = u.username OR cr.assigned_to_id = u.id)
        WHERE $where_clause
        GROUP BY COALESCE(u.full_name, cr.assigned_to)
        ORDER BY deo_name ASC
    ";
    $all_submissions_result = $conn->query($submissions_query);
    
    $grouped_submissions = [];
    if ($all_submissions_result && $all_submissions_result->num_rows > 0) {
        while ($log = $all_submissions_result->fetch_assoc()) {
                    ...
                    <div class="table-responsive">
                        <?php if (count($grouped_submissions) > 0): ?>
                            <?php foreach ($grouped_submissions as $deo_name => $logs): 
                                $total_count = array_sum(array_column($logs, 'record_count'));
                            ?>
                                <div class="work-mgmt-group" style="margin-top: 1.5rem; border: 1px solid #ddd; border-radius: 8px; overflow: hidden;">
                                    <h4 style="background: #eef2f7; padding: 10px 15px; margin: 0; font-size: 1rem; color: var(--primary);">
                                        <?php echo htmlspecialchars($deo_name); ?> (<?php echo $total_count; ?> Records Completed)
                                    </h4>
                                    <table>
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            <?php foreach ($logs as $log): ?>
                                                <tr>
                                                    ...
                                                    <td><?php echo date('d M, Y h:i A', strtotime($log['latest_time'])); ?></td>
                                                </tr>
                                            <?php endforeach; ?>
                                        </tbody>
                                    </table>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <p style="text-align: center; color: var(--secondary);">No completed records found for current filters.</p>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
