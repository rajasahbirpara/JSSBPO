...
// Recent submissions - from client_records (actual completed work)
$logs_query = "SELECT 
    DATE(updated_at) as submission_date,
    MIN(CAST(record_no AS UNSIGNED)) as first_record,
    MAX(CAST(record_no AS UNSIGNED)) as last_record,
                                ...
                                <div style="flex:1;">
                                    ...
                                    <div style="background: #f0f9ff; padding: 8px; border-radius: 6px; margin-bottom: 6px;">
                                        <div style="font-size: 0.9rem;">
                                            📊 <strong>Completed Records:</strong> <span style="color: #16a34a; font-weight: 700;"><?php echo $log['total_records']; ?></span>
                                        </div>
                                    </div>
                                    <div style="font-size: 0.85rem; margin-top: 6px;">
                                        Record Range: <strong><?php echo $log['first_record']; ?> - <?php echo $log['last_record']; ?></strong>
                                    </div>
                                    <div style="font-size: 0.75rem; color: #94a3b8; margin-top: 6px;">
                                        ⏰ Last activity: <?php echo date('h:i A', strtotime($log['latest_time'])); ?>
                                    </div>
                                </div>
                            </div>
                        <?php endwhile; ?>
                    <?php else: ?><p style="text-align:center; color:#94a3b8;">No submissions yet.</p><?php endif; ?>
                </div>
            </div>
            ...
            recordInput.addEventListener('blur', function() {
                ...
                if(val) {
                    ...
                    fetch('api.php', {
                        method: 'POST',
                        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                        body: 'action=get_image_for_report&record_no=' + encodeURIComponent(val)
                    })
                        .then(response => response.json())
                        .then(data => {
                            loader.style.display = 'none';
                            if(data.status === 'success' && data.image_no) {
