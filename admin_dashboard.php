    ...
    <div id="topControls">
        ...
        <div class="header-bar mb-2">
            ...
            <div class="header-stats">
                ...
                <div class="stat-box" style="background:#dc3545; color:#fff; cursor:pointer;" onclick="$('#criticalErrorsModal').modal('show'); loadCriticalErrors();" title="Click to review critical errors">
                    <span class="stat-number" id="criticalErrorsCount">0</span>
                    <span class="stat-label">⚠️ Reports</span>
                </div>
            </div>
        ...
        <div class="controls-row mb-2">
            <div class="filter-group">
                ...
                <select id="filterStatus" class="form-select form-select-sm" onchange="loadData()">
                    ...
                    <option value="reported">⚠️ Reported Issues</option>
            ...
            <table class="table table-bordered table-sm table-striped" id="dataTable">
                <thead>
                    <tr>
                        ...
                        <th>S.No</th><th>Record No.</th><th>Action</th><th>User</th><th>Status</th><th style="font-size:11px;min-width:110px;">Timestamps</th>
...
<script>
    ...
    function updateCounts() {
        ...
        // Handle multiple date formats: YYYY-MM-DD HH:MM:SS or YYYY-MM-DD
        let todayCompleted = allClientData.filter(r => {
            if (r.row_status !== 'Completed' || !r.updated_at) return false;
            
            // Extract just the date part (YYYY-MM-DD)
            let recordDate = r.updated_at.substring(0, 10);
    ...
    function renderTable() {
        ...
        pageData.forEach((r, i) => {
            ...
            // Check if record_no is invalid (non-numeric)
            let isInvalidRecordNo = !/^[0-9]+$/.test(r.record_no);
            let invalidIndicator = isInvalidRecordNo ? '<span class="badge bg-danger ms-1" title="Invalid: Contains non-numeric characters">⚠️</span>' : '';
            let recordCellClass = isInvalidRecordNo ? 'record-cell invalid-record' : 'record-cell';
            
            // Only allow edit for invalid records
            let editEvents = isInvalidRecordNo ? 
                `ondblclick="openEditRecordModal(${r.id}, '${r.record_no}')" oncontextmenu="showRecordContextMenu(event, ${r.id}, '${r.record_no}', true)" title="Double-click or right-click to fix"` : 
                `oncontextmenu="showRecordContextMenu(event, ${r.id}, '${r.record_no}', false)" title="Valid record number"`;
            
            h += `<tr data-id="${r.id}" data-record-no="${r.record_no}" class="${rowClass}">
                ...
                <td class="readonly text-primary ${recordCellClass}" tabindex="0" 
                    onclick="handleRecordClick(this, '${r.record_no}')" 
                    ${editEvents}
                    onkeydown="if(event.key==='Enter') handleRecordClick(this, '${r.record_no}')"><b>${r.record_no}</b>${invalidIndicator}</td>
                <td class="text-center">${doneBtn}</td>
                <td class="readonly small">${r.username || '-'}</td>
                <td class="readonly">${statusBadge}</td>
                <td class="readonly small" style="font-size:10px;min-width:110px;line-height:1.8;">${(() => { let p=[]; if(r.deo_done_at) p.push('<span style="color:#27ae60;">1st QC: '+fmt12hr(r.deo_done_at)+'</span>'); let qts = r.qc_done_at; if(qts) p.push('<span style="color:#0d6efd;">2nd QC: '+fmt12hr(qts)+'</span>'); let cts = r.completed_at||r.updated_at; if(r.row_status==='Completed'&&cts) p.push('<span style="color:#dc3545;">Done: '+fmt12hr(cts)+'</span>'); return p.join('<br>'); })()}</td>`;
            ...
            ['kyc_number','name','guardian_name','gender','marital_status','dob','address','landmark','city','zip_code','city_of_birth','nationality','photo_attachment','residential_status','occupation','officially_valid_documents','annual_income','broker_name','sub_broker_code','bank_serial_no','second_applicant_name','amount_received_from','amount','arn_no','second_address','occupation_profession','remarks'].forEach(c => {
                 ...
                 if (edited.includes(c)) {
                     cls = 'unsaved-cell'; // Yellow - edited
                 }
                 
                 if(isCompleted) {
                     // Completed: tabindex for navigation, click/focus works, but no contenteditable
                     h += `<td class="completed-cell ${cls}" tabindex="0" data-col="${c}" 
                           onfocus="handleFocusCompleted(this)" onclick="handleFocusCompleted(this)">${cellValue}</td>`;
                 } else {
                     h += `<td contenteditable="true" data-col="${c}" class="${cls}" 
                           onfocus="handleFocus(this)" onblur="saveCell(this)" oninput="validateField(this)">${cellValue}</td>`;
                 }
            });
                ...
                if(r.data.length === 0) {
                    ...
                    r.data.forEach(s=>{
                        let roleBadge = s.role === 'admin' ? '<span class="badge bg-danger">Admin</span>' : 
                                       (s.role === 'supervisor' ? '<span class="badge bg-purple">Supervisor</span>' : 
    ...
    $.post('api.php',{action:'get_image', record_no:rec}, res=>{ 
        if(res.status==='success'){ 
            ...
            let img=$('#zoomImage'); 
            let imgPath = res.image.startsWith('/2/') ? res.image : 'uploads/'+res.image;
...
// ========== CRITICAL ERRORS / ADMIN REPORTS FUNCTIONS ==========

function loadCriticalErrors() {
    $.post('api.php', {action: 'get_admin_reports', status: ''}, function(res) {
        if(res.status === 'success') {
            let html = '';
            let reports = res.reports;
            
            if(reports.length === 0) {
                html = '<div class="text-center text-muted py-4"><i class="fas fa-check-circle fa-3x text-success mb-3"></i><br>No pending reports</div>';
            } else {
                ...
                reports.forEach(r => {
                    let statusBadge = r.status === 'pending' ? '<span class="badge bg-warning">Pending</span>' : 
                                      r.status === 'in_review' ? '<span class="badge bg-info">In Review</span>' : 
                                      '<span class="badge bg-success">Resolved</span>';
                    
                    let roleIcon = r.reported_by_role === 'qc' ? '🔵 QC' : r.reported_by_role === 'deo' ? '🟢 DEO' : '🔴 Admin';
                    
                    let actions = '';
                    if(r.status !== 'resolved') {
                        actions = `<button class="btn btn-success btn-sm" onclick="resolveReport(${r.id})">✅ Resolve</button>`;
                    } else {
                        actions = `<small class="text-muted">By: ${r.resolved_by_name || r.resolved_by}</small>`;
                    }
                    
                    html += `<tr class="${r.status === 'pending' ? 'table-warning' : ''}">
                        <td><strong>${r.record_no}</strong><br><small>${r.name || '--'}</small></td>
                        <td><code>${r.field_name}</code></td>
                        <td>${r.issue_details}</td>
                        <td>${roleIcon}<br><small>${r.reported_by_name || r.reported_by}</small></td>
                        <td><small>${r.reported_from}</small></td>
                        <td><small>${r.created_at}</small></td>
                        <td>${statusBadge}</td>
                        <td>${actions}</td>
                    </tr>`;
            ...
            $('#criticalErrorsContent').html(html);
            
            // Update count
            let pendingCount = reports.filter(r => r.status !== 'resolved').length;
            $('#criticalErrorsCount').text(pendingCount);
        }
    }, 'json');
}

function resolveReport(reportId) {
    let notes = prompt('Enter resolution notes (optional):');
    if(notes === null) return; // Cancelled
    
    $.post('api.php', {
        action: 'resolve_admin_report',
        report_id: reportId,
...
// Load critical errors count on page load
$(document).ready(function() {
    // Load report counts
    $.post('api.php', {action: 'get_report_counts'}, function(res) {
        if(res.status === 'success') {
            $('#criticalErrorsCount').text(res.counts.pending + res.counts.in_review);
        }
    }, 'json');
});

// ========== END CRITICAL ERRORS FUNCTIONS ==========

</script>

<!-- Critical Errors Review Modal -->
<div class="modal fade" id="criticalErrorsModal" tabindex="-1">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header bg-danger text-white">
                <h5 class="modal-title">⚠️ Critical Errors Review - Reports from DEO/QC</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body" id="criticalErrorsContent" style="max-height:70vh; overflow-y:auto;">
                Loading...
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <button type="button" class="btn btn-primary" onclick="loadCriticalErrors()">🔄 Refresh</button>
            </div>
        </div>
    </div>
</div>
