...
if (isset($_FILES['ocr_image_upload'])) {
    ...
    $safe_name = uniqid('ocr_') . '_' . preg_replace('/[^a-zA-Z0-9._-]/', '', basename($file["name"]));
...
if (!file_exists('uploads/mapping_images')) {
    mkdir('uploads/mapping_images', 0755, true);
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_user'])) {
    ...
    if ($stmt->num_rows > 0) {
        ...
        $ins_stmt->bind_param("ssss", $new_username, $hashed_password, $new_role, $new_fullname);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['assign_records'])) {
    ...
    if (!is_numeric($record_from) || !is_numeric($record_to)) {
        ...
        if ($overlap_result->num_rows > 0) { 
            ...
            if ($stmt->execute()) {
                // Get DEO username for client_records
                $deo_stmt = $conn->prepare("SELECT username FROM users WHERE id = ?");
                $deo_stmt->bind_param("i", $deo_id);
                $deo_stmt->execute();
                $deo_res = $deo_stmt->get_result();
                $deo_username = ($deo_res && $deo_row = $deo_res->fetch_assoc()) ? $deo_row['username'] : '';
                
                $upd_stmt = $conn->prepare("UPDATE client_records SET assigned_to = ?, assigned_to_id = ? WHERE CAST(record_no AS UNSIGNED) BETWEEN ? AND ?");
                $upd_stmt->bind_param("siii", $deo_username, $deo_id, $record_from, $record_to);
                $upd_stmt->execute();
